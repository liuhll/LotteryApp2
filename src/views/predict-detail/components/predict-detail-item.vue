...
import { Scroller, LoadMore,CellBox } from 'vux'
export default {
  props: {
    predictDetailData: {
        type: Object,
  ...
  computed: {
    ...
    currentPredictPreiod() {
      ...
      const startPeriod = currentPredictData.startPeriod.toString().substr(currentPredictData.startPeriod.toString().length - 3,3);
      const endPeriod = currentPredictData.endPeriod.toString().substr(currentPredictData.endPeriod.toString().length - 3,3);
      return  `${startPeriod}期-${endPeriod}期`
    },
    currentPredictData() {
        return this.predictData.currentPredictData.predictedData.replace(/,/g,' ');
    },
    currentSmallPeriod() {
  ...
  methods: {
    nextLotteryCountdown() {
      ...
      if (self.remainSeconds > 0) {
          ...
          self.getPredictDetailData()
        }
             
    },
    getPredictDetailData(needClearCountdownTimeOut) {
        const self = this;
        if (self && !self._isDestroyed) {
            self.$store.dispatch('GetPredictDetailData',self.predictData.normId).then(result => {
                if(result.finalLotteryData.isLotteryData) {
                    self.predictData = result;
                    self.remainSeconds = result.finalLotteryData.remainSeconds;
                    ...
                    self.nextLotteryCountdown();
                     ...
                     setTimeout(self.getPredictDetailData, 3000,needClearCountdownTimeOut);
                       if (needClearCountdownTimeOut && self.countdownTimeOut) {
                          clearTimeout(self.countdownTimeOut);
                          return;
                       }
    ...
    onVisibilityChange() {
        this.getPredictDetailData(true);
    },
